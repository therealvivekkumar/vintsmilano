...
export function Navbar({ onLogoClick }: { onLogoClick?: () => void } = {}) {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-[200] transition-all duration-500 border-b border-transparent py-2 md:py-2.5 ${
        scrolled ? "bg-background/95 backdrop-blur-md border-border/50 md:py-0.5 shadow-sm" : "bg-transparent"
      }`}
    >
      <div className="container mx-auto px-6 lg:px-12 relative flex items-center justify-between min-h-11 md:min-h-14">
        ...
        <a href="/" onClick={handleLogoClick} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center" aria-label="Go to home">
          <img
            ...
            alt="Vints Milano"
            className="h-11 md:h-14 w-auto transition-opacity duration-300"
          />
        </a>
